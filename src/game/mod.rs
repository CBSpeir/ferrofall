...
pub(crate) use board::{BOARD_HEIGHT, BOARD_WIDTH, Board, VISIBLE_HEIGHT, VISIBLE_TOP};
pub(crate) use piece::{Point, Tetromino, preview_offsets};
pub(crate) use scoring::{ClearResult, Spin};
...
pub(crate) struct GameConfig {
    pub(crate) das_ticks: u32,
    pub(crate) arr_ticks: u32,
    pub(crate) lock_delay_ticks: u32,
    pub(crate) max_lock_resets: u8,
    pub(crate) soft_drop_units: u64,
}
...
impl Game {
    pub(crate) fn new(config: GameConfig, seed: u64) -> Self {
    ...
    pub(crate) fn active_kind(&self) -> Tetromino {
        self.active.kind
    }

    pub(crate) fn active_blocks(&self) -> [Point; 4] {
        self.active.blocks()
    }

    pub(crate) fn ghost_blocks(&self) -> [Point; 4] {
    ...
    pub(crate) fn held_piece(&self) -> Option<Tetromino> {
        self.hold
    }

    pub(crate) fn hold_available(&self) -> bool {
        !self.hold_used && !self.game_over
    }

    pub(crate) fn next_pieces(&self) -> impl Iterator<Item = Tetromino> + '_ {
    ...
    pub(crate) fn drain_events(&mut self) -> impl Iterator<Item = GameEvent> + '_ {
        self.events.drain(..)
    }
