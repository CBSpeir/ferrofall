...
pub(crate) const BOARD_HEIGHT: usize = 40;
pub(crate) const VISIBLE_HEIGHT: usize = 20;
pub(crate) const VISIBLE_TOP: usize = BOARD_HEIGHT - VISIBLE_HEIGHT;
...
impl Board {
    pub(crate) fn cell(&self, x: usize, y: usize) -> Option<Tetromino> {
        self.cells[y][x]
    }

    pub(crate) fn collides(&self, piece: ActivePiece) -> bool {
        piece
            .blocks()
            .into_iter()
    ...
    pub(crate) fn point_is_occupied(&self, point: Point) -> bool {
        if point.x < 0
    ...
    pub(crate) fn lock(&mut self, piece: ActivePiece) {
    ...
    pub(crate) fn clear_full_rows(&mut self) -> Vec<usize> {
    ...
    #[cfg(test)]
    pub(crate) fn set_cell(&mut self, x: usize, y: usize, value: Option<Tetromino>) {
