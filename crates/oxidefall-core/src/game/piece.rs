...
impl ActivePiece {
    pub const fn spawn(kind: Tetromino) -> Self {
        Self {
            kind,
    ...
    pub fn rotated(self, rotation: Rotation, dx: i32, dy: i32) -> Self {
        Self {
            rotation,
            origin: Point::new(self.origin.x + dx, self.origin.y + dy),
            ..self
...
pub fn preview_offsets(kind: Tetromino) -> [Point; 4] {
...
pub fn kick_tests(kind: Tetromino, from: Rotation, to: Rotation) -> [(i32, i32); 5] {
    if kind == Tetromino::O {
        return [(0, 0); 5];
    }
